...
const VERIFY_TEMPLATE = `ref_user_name,ref_phone,ref_challenge,ref_entry_fee,ref_strava_url,ref_submitted_at,submission_id,participant_id,user_id,challenge_id,action,amount,reward_type,win_code,note,rejection_reason
John Doe,9876543210,Morning Rush,79,https://strava.app.link/xxx,2026-05-01,SUB_UUID,PART_UUID,USER_UUID,CHAL_UUID,verify,500,prize,P1,Winner,
Jane Doe,9876543211,Morning Rush,79,https://strava.app.link/yyy,2026-05-01,SUB_UUID2,PART_UUID2,USER_UUID2,CHAL_UUID2,reject,,,,,Run paused multiple times`
...
export function BulkVerify() {
  ...
  function downloadTemplate() {
    ...
    a.href = url; a.download = 'zelth_bulk_verify_template.csv'; a.click()
  ...
  function downloadPendingCSV() {
    ...
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'zelth_pending_submissions.csv'; a.click()
    URL.revokeObjectURL(url)
  ...
  async function handleProcess() {
    ...
    const verifyWithoutAmount = rows.filter(r => r.action === 'verify' && (!r.amount || Number(r.amount) <= 0))
    if (verifyWithoutAmount.length > 0) {
      const confirm = window.confirm(`⚠️ ${verifyWithoutAmount.length} verify rows have no amount — wallet will NOT be credited for these. Continue anyway?`)
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (row.action === 'verify') {
          ...
          res[i] = { row, status: 'success', message: `✓ Verified${row.amount ? ` + ₹${row.amount} credited` : ''}` }
    ...
    toast(`Done! ${successCount} processed, ${errorCount} failed`, successCount > 0 ? 'success' : 'error')
    ...
    exportCsv(resultRows as Record<string, unknown>[], 'zelth_bulk_verify_results')
  }
    ...
    <div style={{ padding: 24 }}>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">Process multiple submissions at once via CSV</div>
        </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ fontWeight: 600, marginBottom: 12 }}>Step 1 — Get Pending Submissions</div>
        ...
        {pendingSubs.length > 0 && (
          <div className="table-wrap" style={{ marginTop: 16, maxHeight: 240, overflow: 'auto' }}>
            <table>
              <thead>
                ...
                {filteredPendingSubs.map((s: any) => (
                  <tr key={s.id}>
                    <td>
                      ...
                      <div className="mono" style={{ color: 'var(--text3)', fontSize: 11 }}>{s.users?.phone}</div>
                    </td>
                    <td>{s.challenges?.title}</td>
                    <td>
                      <a href={s.strava_url} target="_blank" rel="noopener noreferrer" className="link-cell" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <ExternalLink size={11} /> View
                      </a>
                    </td>
                    <td style={{ color: 'var(--text2)', fontSize: 12 }}>{new Date(s.submitted_at).toLocaleString('en-IN')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ fontWeight: 600, marginBottom: 12 }}>Step 2 — Upload Filled CSV</div>
        <div
          style={{ border: '2px dashed var(--border2)', borderRadius: 8, padding: '24px 20px', textAlign: 'center', cursor: 'pointer' }}
          onClick={() => fileRef.current?.click()}
        >
          <Upload size={28} color="var(--text3)" style={{ marginBottom: 8 }} />
          <div style={{ fontWeight: 500, marginBottom: 4 }}>Drop CSV or click to browse</div>
          <div style={{ color: 'var(--text3)', fontSize: 12 }}>Columns: submission_id, action (verify/reject), amount, win_code, rejection_reason</div>
        ...
        {rows.length > 0 && (
          <div style={{ marginTop: 16 }}>
            <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
              <span style={{ color: 'var(--green)', fontSize: 13 }}>✓ {verifyCount} to verify</span>
              <span style={{ color: 'var(--red)', fontSize: 13 }}>✗ {rejectCount} to reject</span>
            </div>

            <div className="table-wrap" style={{ maxHeight: 200, overflow: 'auto', marginBottom: 16 }}>
              <table>
                <thead>
                  <tr><th>#</th><th>Submission ID</th><th>Action</th><th>Amount</th><th>Reward Type</th><th>Win Code</th><th>Rejection Reason</th></tr>
                </thead>
                <tbody>
                  {rows.map((r, i) => (
                    <tr key={i}>
                      <td style={{ color: 'var(--text3)' }}>{i + 1}</td>
                      <td className="mono" style={{ fontSize: 11 }}>{r.submission_id?.slice(0, 18)}...</td>
                      <td>
                        <span className={`badge ${r.action === 'verify' ? 'badge-verified' : 'badge-rejected'}`}>{r.action}</span>
                      </td>
                      <td style={{ color: 'var(--orange)', fontWeight: 600 }}>{r.amount ? `₹${r.amount}` : '—'}</td>
                      <td><span className="badge badge-pending">{r.reward_type || 'prize'}</span></td>
                      <td className="mono">{r.win_code || '—'}</td>
                      <td style={{ color: 'var(--text2)', fontSize: 12 }}>{r.rejection_reason || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', padding: 10 }} onClick={handleProcess} disabled={processing}>
              {processing ? `Processing... ${progress}%` : `🚀 Process ${rows.length} Submissions`}
            </button>

            {processing && (
              <div style={{ marginTop: 12 }}>
                <div style={{ height: 4, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${progress}%`, background: 'var(--orange)', transition: 'width 0.3s' }} />
                </div>
                <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 4, textAlign: 'center' }}>{progress}% — notifications fire automatically</div>
              </div>
            )}
          </div>
        )}
      ...
      {results.length > 0 && (
        <div className="card" style={{ padding: 0 }}>
            ...
            {errorCount > 0 && (
              <button className="btn btn-ghost btn-sm" onClick={() => exportCsv(
                results
                  ...
                  .map(r => ({ ...r.row, error: r.message })) as Record<string, unknown>[],
                'zelth_failed_rows'
              )}>
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>#</th><th>Submission ID</th><th>Action</th><th>Status</th><th>Message</th></tr></thead>
              <tbody>
                {results.map((r, i) => (
                  <tr key={i}>
                    <td style={{ color: 'var(--text3)' }}>{i + 1}</td>
                    <td className="mono" style={{ fontSize: 11 }}>{r.row.submission_id?.slice(0, 18)}...</td>
                    <td><span className={`badge ${r.row.action === 'verify' ? 'badge-verified' : 'badge-rejected'}`}>{r.row.action}</span></td>
                    <td>
