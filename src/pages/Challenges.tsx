...
export function Challenges() {
  ...
  async function load() {
    ...
    try {
      const [
        ...
        adminDb("select", {
          table: "challenges",
          columns: "*, challenge_types(name, display_name)",
        ...
        supabase
          ...
          .select("id, name, description, is_active, prizes")
          .eq("is_active", true)
          .order("created_at"),
        supabase
          .from("challenge_prize_pools")
          .select("challenge_id, pool, generation, is_active")
          .eq("is_active", true),
      ]);
      ...
      setChallenges(
        (ch || []).map((c: any) => ({
          ...
          min_target: Number(c.min_target),
          target_unit: c.target_unit,
          sort_order: c.sort_order,
          total_slots: c.total_slots,
          challenge_type:
            c.challenge_types?.display_name || c.challenge_types?.name || "—",
  ...
  function openEdit(c: Challenge) {
    setForm({
      ...
      min_target: String(c.min_target),
      target_unit: c.target_unit,
      sort_order: String(c.sort_order),
      total_slots: String(c.total_slots),
      rules: Array.isArray(c.rules) ? c.rules.join("\n") : "",
      cashback_tiers: c.cashback_tiers
        ? typeof c.cashback_tiers === "string"
          ? c.cashback_tiers
          : JSON.stringify(c.cashback_tiers, null, 2)
        : "",
  ...
  async function handleSave() {
    ...
    try {
      ...
      let rules: string[] = [];
      try {
        cashback = JSON.parse(form.cashback_tiers);
      } catch {}
      try {
      ...
      const payload: Record<string, unknown> = {
        ...
        ...(form.challenge_type_id
          ? { challenge_type_id: form.challenge_type_id }
          : {}),
      };
      ...
      if (editId) {
        ...
        const res = await adminDb("insert", {
          table: "challenges",
          data: payload,
        });
        ...
        if (form.prize_pool_template_id) {
          ...
          if (!tpl?.prizes?.length) {
            toast(
              "Challenge created but template has no prizes defined. Update via Set Pool.",
              "error",
            );
          } else {
    ...
    try {
      ...
      if (poolModal.has_pool) {
        await adminDb("update", {
          ...
          data: {
            pool,
            generation: poolModal.pool_generation + 1,
            is_active: true,
          },
    ...
    <div style={{ padding: 24 }}>
                ...
                {challenges.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td style={{ color: "var(--green)" }}>
                      ₹{c.prize_pool.toLocaleString("en-IN")}
                    </td>
                    <td>
                      {c.has_pool && c.pool && c.pool.length > 0 ? (
                        <span
                          ...
                          title={c.pool
                            .map((p) => `₹${p.toLocaleString("en-IN")}`)
                            .join(", ")}
                        >
                          ₹{Math.min(...c.pool).toLocaleString("en-IN")} – ₹
                          {Math.max(...c.pool).toLocaleString("en-IN")}
                        </span>
                      ) : (
                    ...
                    <td style={{ color: "var(--text2)" }}>
                      {c.personal_window_days}d
                    </td>
                    <td>
                      <span
                        className={`badge ${c.is_active ? "badge-verified" : "badge-rejected"}`}
                      >
        ...
        <div className="modal-overlay" onClick={() => setModal(null)}>
          ...
          >
            <div className="modal-title">
              {modal === "create" ? "➕ New Challenge" : "✏️ Edit Challenge"}
            </div>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => f("entry_fee", e.target.value)}
                    placeholder="79"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => f("prize_pool", e.target.value)}
                    placeholder="5000"
                  />
                </div>
              </div>
                ...
                <>
                  <div>
                    ...
                    <select
                      ...
                      style={{ width: "100%" }}
                      value={form.challenge_type_id}
                      onChange={(e) => f("challenge_type_id", e.target.value)}
                    >
                  ...
                  <div>
                    <label className="label">
                      Prize Pool Template (optional)
                    </label>
                    <select
                      className="input filter-select"
                      style={{ width: "100%" }}
                      value={form.prize_pool_template_id}
                      onChange={(e) =>
                        f("prize_pool_template_id", e.target.value)
                      }
                    >
              ...
              >
                <div>
                  ...
                  <input
                    className="input"
                    type="number"
                    value={form.personal_window_days}
                    onChange={(e) => f("personal_window_days", e.target.value)}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => f("min_target", e.target.value)}
                  />
                </div>
                <div>
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => f("total_slots", e.target.value)}
                  />
                </div>
                <div>
      ...
      {poolModal && (
        <div className="modal-overlay" onClick={() => setPoolModal(null)}>
          ...
          >
            <div className="modal-title">
              💰 Set Prize Pool — {poolModal.title}
            </div>
            <div style={{ display: "grid", gap: 12 }}>
              {poolModal.has_pool && poolModal.pool && (
                ...
                >
                  <div style={{ color: "var(--text3)", marginBottom: 4 }}>
                    Current pool (Gen {poolModal.pool_generation})
                  </div>
                  <div
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setPoolForm((prev) => ({ ...prev, prizes: e.target.value }))
                  }
