...
import {
  RefreshCw, ChevronDown, ChevronUp, ExternalLink, Edit2, Check,
...
export function Coach() {
  ...
  async function handleProcess() {
    ...
    const toActivate = csvRows.filter(r => (r.coach_dashboard_url || '').trim() !== '')
    const toSkip = csvRows.filter(r => (r.coach_dashboard_url || '').trim() === '')
    ...
    const res: CoachResult[] = csvRows.map(row => ({ row, status: 'pending' }))
    setCsvResults([...res])
    ...
    for (let i = 0; i < csvRows.length; i++) {
      ...
      if (!url) {
        ...
        res[i] = { row, status: 'error', message: 'URL must start with http:// or https://' }
      } else if (!row.user_id) {
  ...
  const csvActivateCount = csvRows.filter(r => (r.coach_dashboard_url || '').trim() !== '').length
  const csvSkipCount = csvRows.filter(r => (r.coach_dashboard_url || '').trim() === '').length
  const csvSuccessCount = csvResults.filter(r => r.status === 'success').length
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* ── CSV Bulk Activation ─────────────────────────────────────────── */}
      <div style={{ color: 'var(--text3)', fontSize: 11, fontWeight: 600, letterSpacing: '0.8px', textTransform: 'uppercase', marginBottom: 8 }}>
        Bulk Activation via CSV
      </div>

      {/* Step 1 — Get pending as CSV */}
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ fontWeight: 600, marginBottom: 12 }}>Step 1 — Download Pending Users</div>
        <p style={{ color: 'var(--text2)', fontSize: 13, marginBottom: 12 }}>
          Download the pending list, fill in <code style={{ background: 'var(--bg3)', padding: '1px 5px', borderRadius: 4 }}>coach_dashboard_url</code> for each user, then upload back.
        </p>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        >
          ...
          <div style={{ color: 'var(--text3)', fontSize: 12 }}>
            Columns: user_id, coach_dashboard_url (rows with empty URL are skipped)
          </div>
          <input
            ref={fileRef}
            type="file"
                ...
                <tbody>
                  {csvRows.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ color: 'var(--text2)' }}>{r.ref_goal || '—'}</td>
                      <td className="mono" style={{ fontSize: 11, color: 'var(--text3)' }}>
                        {r.user_id ? `${r.user_id.slice(0, 12)}…` : '—'}
                      </td>
                      <td style={{ fontSize: 12 }}>
                        {r.coach_dashboard_url
                          ? <span style={{ color: 'var(--blue)' }}>{r.coach_dashboard_url.slice(0, 40)}{r.coach_dashboard_url.length > 40 ? '…' : ''}</span>
                          : <span style={{ color: 'var(--text3)' }}>— (will skip)</span>
                        }
                      </td>
                    </tr>
                  ))}
      ...
      {csvResults.length > 0 && (
        <div className="card" style={{ padding: 0, marginBottom: 28 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontWeight: 600 }}>Results</span>
            {csvSuccessCount > 0 && (
              <span style={{ color: 'var(--green)', fontSize: 12 }}>✓ {csvSuccessCount} activated</span>
            )}
            {csvErrorCount > 0 && (
              <span style={{ color: 'var(--red)', fontSize: 12 }}>✗ {csvErrorCount} failed</span>
            )}
            {csvErrorCount > 0 && (
              <button
                ...
                {csvResults.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      ...
                      {r.status === 'skipped' && <span style={{ color: 'var(--text3)', fontSize: 12 }}>—</span>}
                      {r.status === 'pending' && <Clock size={14} color="var(--text3)" />}
                    </td>
                    <td style={{
                      fontSize: 12,
                      color: r.status === 'error' ? 'var(--red)' : r.status === 'success' ? 'var(--green)' : 'var(--text3)',
                    }}>
                      {r.message || '—'}
                    </td>
                  </tr>
